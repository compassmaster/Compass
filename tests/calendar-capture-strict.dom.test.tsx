...
function StrictHarness() {
  ...
  return <><button type="button" onClick={()=>setVisible(value=>!value)}>{visible?'別タブへ':'会話へ戻る'}</button>{visible&&<ConversationTab session={session} onSessionChange={setSession} scrollPosition={0} onScrollPositionChange={()=>undefined} onNavigateToLog={()=>undefined} onNavigateToRecord={()=>undefined} onNavigateToSleep={()=>undefined} onNavigateToPrediction={()=>undefined} onNavigateToCompassMap={()=>undefined} onNavigateToDetails={()=>undefined} onNavigateToWeather={()=>undefined} onNavigateToBackup={()=>undefined} onCaptureCommitRequest={()=>({ok:false,failure:{code:'x',message:'x',failedAt:new Date().toISOString(),retryable:false}})} onNavigateToCalendarRecord={()=>undefined} onCalendarCommit={commit}/>}</>;
}

describe('Calendar capture StrictMode and receipt lifetime',()=>{
  it('commits while ConversationTab is unmounted, then shows receipt, closes it, and starts another event',async()=>{const user=userEvent.setup();render(<StrictMode><StrictHarness/></StrictMode>);await user.click(screen.getByRole('button',{name:'この内容で保存'}));await user.click(screen.getByRole('button',{name:'別タブへ'}));await new Promise(resolve=>setTimeout(resolve,20));await user.click(screen.getByRole('button',{name:'会話へ戻る'}));const receipt=await screen.findByRole('heading',{name:'予定を保存しました'});expect(document.activeElement).toBe(receipt.closest('section'));await user.click(screen.getByRole('button',{name:'閉じる'}));await waitFor(()=>expect(document.activeElement).toBe(screen.getByLabelText('自由に書く')));const composer=screen.getByLabelText('自由に書く');await user.type(composer,'予定を追加したい');await user.click(screen.getByRole('button',{name:'送信'}));await waitFor(()=>expect(screen.getByText('予定名は何ですか？')).toBeTruthy());});
});
