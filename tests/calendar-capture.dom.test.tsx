...
describe('Calendar Conversation Capture DOM integration', () => {
  it('formats a timed Candidate in Japanese without exposing ISO or timezone data', () => {
    ...
    render(<CalendarCaptureCard capture={capture} onAnswer={()=>undefined} onConfirm={()=>undefined} onBeginEdit={()=>undefined} onApplyEdit={()=>undefined} onReject={()=>undefined} onCancel={()=>undefined} onCommit={()=>undefined} onNavigate={()=>undefined} onDismissReceipt={()=>undefined}/>);
  ...
  it('asks one question, shows excerpt, permits explicit no-edit confirmation, and only then saves', async () => {
    ...
    await user.click(screen.getByRole('button',{name:'カレンダーに追加'})); await user.click(screen.getByRole('button',{name:'カレンダーに追加'})); expect(save).toHaveBeenCalledTimes(1);
  });
