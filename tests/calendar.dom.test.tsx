...
describe('Calendar DOM integration', () => {
  it('focuses a receipt target and reports edit, cancel, and delete so its stale receipt can be dismissed', async () => {
    ...
    await waitFor(()=>expect(document.activeElement).toBe(screen.getByText('会話予定').closest('article'))); expect(consumed).toHaveBeenCalled(); await user.click(screen.getByRole('button',{name:'取消にする'})); await user.click(screen.getByRole('button',{name:'予定に戻す'})); await user.click(screen.getByRole('button',{name:'完了にする'})); await user.click(screen.getByRole('button',{name:'予定に戻す'})); await user.click(screen.getByRole('button',{name:'編集'})); await user.clear(screen.getByRole('textbox',{name:'予定名'})); await user.type(screen.getByRole('textbox',{name:'予定名'}),'編集済み'); await user.click(screen.getByRole('button',{name:'変更を保存'})); await user.click(screen.getByRole('button',{name:'削除'})); await user.click(screen.getByRole('button',{name:'削除を確定する'})); expect(changed.mock.calls.every(([id])=>id===created.record.id)).toBe(true); expect(changed).toHaveBeenCalledTimes(6);
  });
  ...
  it('navigates selectedDate, initializes the form date from it, and shows only its Agenda with a count', async () => {
    ...
    expect(screen.getByText('翌日の予定')).toBeTruthy(); expect(screen.getByRole('heading', { name: /のAgenda/ }).textContent).toContain('1件');
    await user.click(screen.getByRole('button', { name: '前の日' })); expect(screen.queryByText('翌日の予定')).toBeNull(); expect(screen.getByRole('heading', { name: /のAgenda/ }).textContent).toContain('0件');
    await user.click(screen.getByRole('button', { name: '今日' }));
  });
  it('creates MANUAL ALL_DAY, edits with title focus, completes, cancels, and reopens with record focus', async () => {
    const user = await create(); expect(screen.getByText('入力元: 手入力')).toBeTruthy(); expect(screen.getByText('状態: 予定')).toBeTruthy();
    ...
    const article = screen.getByText('定期診察').closest('article')!; await waitFor(() => expect(document.activeElement).toBe(article));
    await user.click(within(article).getByRole('button', { name: '完了にする' })); expect(screen.getByText('状態: 完了')).toBeTruthy(); await waitFor(() => expect(document.activeElement).toBe(article));
    await user.click(within(article).getByRole('button', { name: '予定に戻す' })); expect(screen.getByText('状態: 予定')).toBeTruthy();
    await user.click(within(article).getByRole('button', { name: '取消にする' })); expect(screen.getByText('状態: 取消')).toBeTruthy();
    await user.click(within(article).getByRole('button', { name: '予定に戻す' })); expect(screen.getByText('状態: 予定')).toBeTruthy(); await waitFor(() => expect(document.activeElement).toBe(article));
  });
  ...
  it('focuses the invalid date field and retains input when the range is invalid', async () => {
    const user = userEvent.setup(); render(<CalendarTab service={service} />); await user.type(screen.getByRole('textbox', { name: '予定名' }), '不正範囲');
    const end = screen.getByLabelText('終了日'); await user.clear(end); await user.type(end, '2000-01-01'); await user.click(screen.getByRole('button', { name: '予定を作成' }));
    await waitFor(() => expect(document.activeElement).toBe(end)); expect(screen.getByRole('alert').textContent).toContain('開始と終了'); expect((screen.getByRole('textbox', { name: '予定名' }) as HTMLInputElement).value).toBe('不正範囲');
  });
  ...
  it('retains cards and restores delete focus on status, delete, and subsequent list failures', async () => {
    ...
    const opener = screen.getByRole('button', { name: '削除' }); await user.click(opener); vi.spyOn(service, 'delete').mockReturnValueOnce({ ok: false, reason: 'PERSISTENCE_FAILED' }); await user.click(screen.getByRole('button', { name: '削除を確定する' })); expect(screen.getByText('保持する予定')).toBeTruthy(); await waitFor(() => expect(document.activeElement).toBe(opener));
