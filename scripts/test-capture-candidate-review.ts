...
const resetDuringCommit = transitionConversationSession(requested.session, { type: 'RESET' });
assert.equal(resetDuringCommit.activeCaptureCandidate, requested.session.activeCaptureCandidate, 'free-conversation reset preserves the active Capture candidate');
assert.equal(requestActiveCaptureCandidateCommit(resetDuringCommit, now).commitRequest, undefined, 'reset does not unlock a duplicate Capture commit');
...
assert.equal(transitionConversationSession(session,{type:'RESET'}).activeCaptureCandidate,session.activeCaptureCandidate);
...
assert.equal(synchronizeCaptureCommitRequestGuard(guard, null).requestIssued, false, 'explicit candidate removal clears guard');
assert.equal(synchronizeCaptureCommitRequestGuard(guard, make('same-id')).requestIssued, false, 'same ID can request after explicit candidate removal');
