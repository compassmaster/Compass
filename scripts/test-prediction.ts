...
const nonRain = run(location, [forecast('dry', 0, '2026-07-29T00:00:00Z')], rain);
assert.equal(nonRain.status, 'RAIN_NOT_EXPECTED');
assert.equal(nonRain.dataConfidence, 'HIGH');
assert.equal(nonRain.predictionConfidence, 'MEDIUM');
assert.match(nonRain.summary, /^もし予報どおり雨が降らなければ/);
