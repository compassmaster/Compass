...
#MVP-09確認中に見つかった、旧デモUserModelとEvidenceなし理解表示の矛盾を修正した。これはデータ初期化ではなく、旧デモデータの一度限りのマイグレーションとEvidence不変条件の追加である。

- **完了日**: 2026-07-21
- **完了状態**: Done
- **設計判断**: `localStorage.clear()` は使わず、`compass_user_model` の旧デモUserModelのみを削除対象にする。DailyLog / Insight / Candidate / historyは保持する。AI生成理解はEvidenceが0件なら保存・表示対象外にする。

### Acceptance Criteria確認

- [x] 旧デモUserModelだけを一度限りのmigrationで削除する。
- [x] migration済みflagを保存し、毎回削除処理を走らせない。
- [x] DailyLogなど正式なユーザーデータは削除しない。
- [x] Evidence 0件のCandidateはUserModelへ適用できない。
- [x] 適用拒否時にUserModel / Candidate状態 / historyを中途半端に更新しない。
- [x] Evidence 0件の既存UserModel理解はCompass Mapに表示しない。
- [x] Evidence 1件以上の理解は表示する。
- [x] 完全な空状態でも自然な空表示になる。
