...
UserModelは、正式な理解パイプラインの最後に位置する。UserModelはUnderstanding Objectの集約であり、Candidate Responseを直接保持しない。
...
- ユーザー確認前の候補を反映しない。
- Candidate Responseを直接保持しない。
- Candidate Responseから直接既存Hypothesisフィールドを書き換えない。
- AGREE回答だけでUnderstanding Status Confirmedにしない。
- LLMによる直接更新を受け付けない。
...
また、正式なUnderstanding Candidate ResponseからUnderstanding Objectを生成し、UserModelへ保存するフローも未実装である。Candidate Responseから直接既存Hypothesis型UserModelを書き換えることはしない。
...
目標アーキテクチャでは、UserModelは複数のUnderstanding Objectを保持する。AGREEから生成されたObjectも初期maturityはHypothesisであり、確認済み回答とConfirmed maturityを混同しない。
...
UnderstandingObject
        ↓
UnderstandingRepository / UserModel boundary
```
...
- Candidate Responseが存在し、回答が `AGREE` である。
- `PARTIALLY_DISAGREE` と `UNSURE` はUnderstanding Objectを生成せず、UserModelへ反映しない。
- Candidate Responseから直接既存Hypothesisフィールドを書き換えない。
- Understanding Object生成とUserModel保存は別責務にする。
- AGREE回答だけでUnderstanding Status Confirmedにしない。

既存Hypothesis型UserModelは段階移行中である。この設計PRではUserModelコードを変更しない。
