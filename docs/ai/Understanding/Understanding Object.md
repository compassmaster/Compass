...
D-0009により、Formal UserModelとの関係は以下のように設計され、Phase Aとして型・Repository・Reconciler・Resolverは実装済みである。ただしApp統合とConsumer接続は未実装である。
...
- Formal UserModelは `compass_formal_user_model_v1` に旧 `compass_user_model` とは別保存する。TypeScript型、型ガード、Repository、Reconciler、Resolver、保存処理は実装済みである。
