...
- unknown responseについてroot/error/座標/timezone/daily/date/配列長/number-or-nullを検証する。HTTP、JSON、network、10秒timeout、要求日不一致は保存前に失敗する。
- 正規化後だけ`ObservedWeatherRecordRepository.save()`し、`sourceType: HISTORICAL`を付ける。Provider DTOは保存せずnullを推測しない。
- UI表記は「過去の推定気象データ」。同日最新選択は`Date.parse(source.fetchedAt)`、`Date.parse(createdAt)`、IDの順で、最大7日を表示する。
