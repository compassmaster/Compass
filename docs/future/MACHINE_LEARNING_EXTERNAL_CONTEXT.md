...
この文書全体はFuture Conceptであり、Accepted ADR、現在仕様、実装済み機能として扱わない。コード変更も行わない。
...
具体的なAPI採用は未決定である。Open-Meteo等は候補に留め、採用する場合はADRでデータ取得範囲、保存範囲、更新頻度、失敗時挙動、プライバシー影響を決定する。
