...
import { availableTimeZones, calendarEventOccursOnDate, instantToLocalDateTime, localDateTimeToOffsetInstant, localToday, moveLocalDate } from './calendarDateTime.ts';
...
export function CalendarTab({ service = defaultService }: { service?: CalendarEventApplicationService }) {
  ...
  const [message, setMessage] = useState(loaded.failed ? '予定を読み込めませんでした。表示中の予定は保持されています。' : '');
  ...
  const selectDate = (date: string) => { setSelectedDate(date); if (!editing) setDraft(draftFor(date)); };
  const makeInput = (): CorrectCalendarEventInput | null => {
    const common = { title: draft.title.trim(), note: draft.note.trim() || undefined };
    if (draft.timeKind === 'ALL_DAY') return { ...common, timeKind: 'ALL_DAY', startDate: draft.startDate, endDate: draft.endDate };
    ...
    return startsAt && endsAt ? { ...common, timeKind: 'TIMED', startsAt, endsAt, timeZone: draft.timeZone } : null;
  };
  const submit = (event: FormEvent) => {
    event.preventDefault(); const input = makeInput();
    if (!input) { setMessage('日時とタイムゾーンを確認してください。夏時間で存在しない・重複する時刻は保存できません。'); return; }
    const result = editing ? service.correct(editing, input) : service.create({ ...input, source: 'MANUAL' });
    if (!result.ok) { setMessage('予定を保存できませんでした。表示中の予定と入力内容は保持されています。'); return; }
    const targetId = editing; resetCreateForm(); refresh();
    afterRender(() => targetId ? recordRefs.current.get(targetId)?.focus() : agendaHeadingRef.current?.focus());
  };
  ...
  return <section className="calendar" aria-labelledby="calendar-heading">
    ...
    <div className="calendar-date-navigation" aria-label="Agendaの日付"><button type="button" onClick={() => selectDate(moveLocalDate(selectedDate, -1))}>前の日</button><button type="button" onClick={() => selectDate(localToday())}>今日</button><label>表示する日<input type="date" value={selectedDate} onChange={(event) => selectDate(event.target.value)} /></label><button type="button" onClick={() => selectDate(moveLocalDate(selectedDate, 1))}>次の日</button></div>
    ...
    <form className="calendar-form" onSubmit={submit}><h3>{editing ? '予定を編集' : '予定を作成'}</h3><label>予定名<input ref={titleRef} required value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} /></label><label>メモ（任意）<textarea value={draft.note} onChange={(event) => setDraft({ ...draft, note: event.target.value })} /></label><fieldset><legend>予定の種類</legend><label><input type="radio" checked={draft.timeKind === 'ALL_DAY'} onChange={() => setDraft({ ...draft, timeKind: 'ALL_DAY' })} />終日</label><label><input type="radio" checked={draft.timeKind === 'TIMED'} onChange={() => setDraft({ ...draft, timeKind: 'TIMED' })} />時刻指定</label></fieldset>
      {draft.timeKind === 'ALL_DAY' ? <div className="calendar-fields"><label>開始日<input required type="date" value={draft.startDate} onChange={(event) => setDraft({ ...draft, startDate: event.target.value })} /></label><label>終了日<input required type="date" value={draft.endDate} onChange={(event) => setDraft({ ...draft, endDate: event.target.value })} /></label></div> : <><div className="calendar-fields"><label>開始日時<input required type="datetime-local" value={draft.startsAt} onChange={(event) => setDraft({ ...draft, startsAt: event.target.value })} /></label><label>終了日時<input required type="datetime-local" value={draft.endsAt} onChange={(event) => setDraft({ ...draft, endsAt: event.target.value })} /></label></div><label>タイムゾーン<select value={draft.timeZone} onChange={(event) => setDraft({ ...draft, timeZone: event.target.value })}>{availableTimeZones(draft.timeZone).map((zone) => <option key={zone}>{zone}</option>)}</select></label></>}
    ...
    <section className="calendar-list" aria-labelledby="agenda-heading"><h3 id="agenda-heading" ref={agendaHeadingRef} tabIndex={-1}>{selectedDate}のAgenda</h3>{!loaded.failed && agenda.length === 0 ? <p>この日の予定はありません。</p> : agenda.map((record) => <article ref={(node) => { if (node) recordRefs.current.set(record.id, node); }} key={record.id} className={`calendar-event status-${record.status.toLowerCase()}`} tabIndex={-1}><header><h4>{record.title}</h4><strong className="calendar-status">状態: {statusLabels[record.status]}</strong></header><p>{record.timeKind === 'ALL_DAY' ? `終日: ${record.startDate}〜${record.endDate}` : `時刻指定: ${instantToLocalDateTime(record.startsAt, record.timeZone)}〜${instantToLocalDateTime(record.endsAt, record.timeZone)} (${record.timeZone})`}</p><p>入力元: {record.source === 'MANUAL' ? '手入力' : '会話から保存'}</p>{record.note && <p>{record.note}</p>}<div className="calendar-actions"><button type="button" onClick={() => beginEdit(record)}>編集</button>{record.status === 'PLANNED' ? <><button type="button" onClick={() => changeStatus(record, 'complete')}>完了にする</button><button type="button" onClick={() => changeStatus(record, 'cancel')}>取消にする</button></> : <button type="button" onClick={() => changeStatus(record, 'reopen')}>予定に戻す</button>}<button type="button" onClick={(event) => { deleteOpenerRef.current = event.currentTarget; setDeleting(record); }}>削除</button></div></article>)}</section>
