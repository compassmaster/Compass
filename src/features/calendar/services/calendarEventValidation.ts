...
const PROVENANCE_KEYS = ['capturedAt', 'consentedAt', 'extractionMethod', 'extractorVersion', 'sourceExcerpt'];
const hasOnlyKeys = (value: object, keys: string[]) => Object.keys(value).every((key) => keys.includes(key));
const nonBlank = (value: unknown): value is string => typeof value === 'string' && value.trim().length > 0;
const instant = (value: unknown): value is string => nonBlank(value) && Number.isFinite(Date.parse(value));
...
function offsetMatchesTimeZone(value: string, timeZone: string): boolean {
  ...
  const expectedMinutes = Math.round((localAsUtc - Date.parse(value)) / 60_000);
...
function validProvenance(value: unknown, createdAt: string): value is ConversationProvenance {
  ...
  return instant(p.capturedAt) && instant(p.consentedAt) && Date.parse(p.capturedAt) <= Date.parse(p.consentedAt) &&
    Date.parse(p.consentedAt) <= Date.parse(createdAt) && nonBlank(p.extractionMethod) && nonBlank(p.extractorVersion) && nonBlank(p.sourceExcerpt);
}
...
export function isCalendarEventRecord(value: unknown): value is CalendarEventRecord {
  ...
  if (!keys.length || !hasOnlyKeys(value, keys) || !nonBlank(r.id) || !nonBlank(r.title) || (r.note !== undefined && typeof r.note !== 'string') ||
      ...
      !Number.isInteger(r.revision) || (r.revision as number) < 1 || !instant(r.createdAt) || !instant(r.updatedAt) || Date.parse(r.createdAt) > Date.parse(r.updatedAt)) return false;
  ...
  return instant(r.startsAt) && instant(r.endsAt) && isIanaTimeZone(r.timeZone) && Date.parse(r.startsAt) < Date.parse(r.endsAt) &&
