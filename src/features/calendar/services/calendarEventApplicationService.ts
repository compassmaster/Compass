...
export type CalendarMutationResult = { ok: true; record: CalendarEventRecord } | { ok: false; reason: 'INVALID_INPUT' | 'NOT_FOUND' | 'NO_CHANGE' | 'INVALID_TRANSITION' | 'DUPLICATE_ID' | 'PERSISTENCE_FAILED' };
...
export class CalendarEventApplicationService {
  ...
  create(input: CreateCalendarEventInput): CalendarMutationResult {
    ...
    try { this.repository.save(clone(created.record)); return { ok: true, record: clone(created.record) }; } catch (error) { return { ok: false, reason: persistenceReason(error) }; }
  }
  correct(id: CalendarEventId, input: CorrectCalendarEventInput): CalendarMutationResult {
    let existing: CalendarEventRecord | null;
    try { existing = this.repository.getById(id); } catch { return { ok: false, reason: 'PERSISTENCE_FAILED' }; }
  ...
  delete(id: CalendarEventId): CalendarDeleteResult {
    try { if (!this.repository.getById(id)) return { ok: false, reason: 'NOT_FOUND' }; }
    catch { return { ok: false, reason: 'PERSISTENCE_FAILED' }; }
...
const persistenceReason = (error: unknown): 'DUPLICATE_ID' | 'PERSISTENCE_FAILED' =>
  error instanceof CalendarEventRepositoryError && error.code === 'DUPLICATE_ID' ? 'DUPLICATE_ID' : 'PERSISTENCE_FAILED';
