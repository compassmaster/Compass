...
} from '../services/userModelUpdateCandidateService';
import type { UserModelUpdateHistoryEntry } from '../services/userModelUpdateApplicationService.ts';
import { hasEvidenceBackedUnderstanding } from '../services/userModelEvidenceGuards';
import './MapTab.css';
    ...
    <div>
          ...
          <div className="map-grid">
            {pendingCandidates.map((candidate) => (
              <div key={candidate.id} className="map-card">
                ...
                <p className="map-description">
                  根拠件数: {candidate.evidenceRefs.length}件
                </p>
      ...
      <section className="map-section">
        <h3 className="map-subtitle">
          現在Compassが理解していること — 🏠 家の柱
        </h3>

        <div className="map-grid">
          <UnderstandingCard
            title="🌟 大切にしている価値観"
            hypothesis={userModel.longTerm.coreValues}
            targetField="longTerm.coreValues"
            historyEntries={historyEntries}
          />

          <UnderstandingCard
            title="🚀 目指したい未来"
            hypothesis={userModel.longTerm.longTermGoals}
            targetField="longTerm.longTermGoals"
            historyEntries={historyEntries}
          />

          <UnderstandingCard
            title="🌱 あなたの性格傾向"
            hypothesis={userModel.longTerm.personalityTraits}
            targetField="longTerm.personalityTraits"
            historyEntries={historyEntries}
          />
        </div>
      </section>
        ...
        <div className="map-grid">
          <div className="map-card">
            ...
            <p className="mood-status">
              {userModel.shortTerm.currentMood.status}（強度:{' '}
              {userModel.shortTerm.currentMood.intensity}/5）
            </p>
          ...
          <UnderstandingCard
            title="⚠️ 直近の悩み・関心事"
            hypothesis={userModel.shortTerm.immediateConcerns}
          ...
          <UnderstandingCard
            title="🎮 最近のマイブーム"
            hypothesis={userModel.shortTerm.recentInterests}
...
function UnderstandingCard({
  ...
  const relatedHistory = historyEntries.filter(
    (entry) => entry.targetField === targetField,
  );

  const evidenceCount = hypothesis.evidenceList.length;
  const isEvidenceBacked = hasEvidenceBackedUnderstanding(hypothesis);
  ...
  return (
    <div className="map-card">
      ...
      <ul className="map-list">
        {isEvidenceBacked ? (
          hypothesis.value.map((value, index) => (
            <li key={`${title}-${index}`}>{value}</li>
          ))
        ) : (
          <li>まだあなたについて十分な理解を持っていません。</li>
        )}
      </ul>
        ...
        <div className="understanding-detail-body">
          ...
          <p>
            {isEvidenceBacked
              ...
              : 'この領域については、まだあなたについて十分な理解を持っていません。'}
          </p>

          <p>
            まだ仮説です。これはまだ絶対ではありません。今後の記録で変化する可能性があります。
          </p>

          <p>
            {isEvidenceBacked
              ? `確信度の理由: 根拠が${evidenceCount}件観測されたため、現在の確信度として表示しています。`
              : '確信度の理由: 表示できる根拠がまだないため、確信度は表示していません。'}
          </p>
          ...
          {isEvidenceBacked && relatedHistory.length > 0 && (
