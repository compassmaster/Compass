...
import type { BaseLocation } from '../../external-context/location/types/index.ts';
...
export class PredictionQueryService {
  ...
  getTomorrowFatigueOutlook(): TomorrowFatiguePredictionReadModel {
    ...
    const selected = selectLatestForecast(this.forecasts.findByTargetDate(targetDate, location.timezone), targetDate, location);
    ...
    if (precipitation <= 0) return result('RAIN_NOT_EXPECTED', targetDate, location.timezone, selected, null, 'もし予報どおり雨が降らなければ、雨を条件にした疲労の高まりは明日の見通しには含めません。ただし、天気以外の要因による疲労は判断できません。');
    const rainRelationship = this.relationships.getRelationships().cards.find((card) => card.kind === 'RAIN_FATIGUE')!;
    if (rainRelationship.status !== 'RELATIONSHIP_FOUND' || rainRelationship.fatigueDifference === null || rainRelationship.fatigueDifference <= 0) {
      return result('RELATIONSHIP_NOT_SUPPORTED', targetDate, location.timezone, selected, rainRelationship, '明日は雨の予報ですが、雨の日に疲労が高めだったという関係を示す記録はまだ確認できません。');
    }
    return result('OUTLOOK_AVAILABLE', targetDate, location.timezone, selected, rainRelationship, 'もし予報どおり雨になり、これまでと似た傾向が続くなら、明日の疲労は雨でない日より高めになる可能性があります。', 'HIGHER');
  }
}
...
function result(status: TomorrowFatiguePredictionReadModel['status'], targetDate: string, timezone: string, forecast: WeatherForecastSnapshot | null, relationship: RelationshipCardReadModel | null, summary: string, direction: TomorrowFatiguePredictionReadModel['direction'] = null): TomorrowFatiguePredictionReadModel {
  ...
  const precipitation = typeof forecast?.forecastValues.precipitation?.value === 'number' ? forecast.forecastValues.precipitation.value : null;
  return { status, targetDate, timezone, summary, direction, rainExpected: precipitation === null ? null : precipitation > 0, forecastFetchedAt: forecast?.source.fetchedAt ?? null, relationshipStatus: relationship?.status ?? 'NOT_READ', relationshipAnalysisConfidence: relationship?.analysisConfidence ?? null, forecastPrecipitation: precipitation, relationshipFatigueDifference: relationship?.fatigueDifference ?? null, dataConfidence: dataConfidence(forecast, relationship), predictionConfidence, sourceRecordIds: { forecastSnapshotIds: forecast ? [forecast.id] : [], relationshipDailyLogIds: relationship ? [...relationship.sourceRecordIds.dailyLogIds].sort((a, b) => a.localeCompare(b)) : [], relationshipWeatherRecordIds: relationship ? [...relationship.sourceRecordIds.weatherRecordIds].sort((a, b) => a.localeCompare(b)) : [] } };
}
function empty(status: TomorrowFatiguePredictionReadModel['status'], summary: string): TomorrowFatiguePredictionReadModel { return { status, targetDate: null, timezone: null, summary, direction: null, rainExpected: null, forecastFetchedAt: null, relationshipStatus: 'NOT_READ', relationshipAnalysisConfidence: null, forecastPrecipitation: null, relationshipFatigueDifference: null, dataConfidence: 'LOW', predictionConfidence: 'LOW', sourceRecordIds: { forecastSnapshotIds: [], relationshipDailyLogIds: [], relationshipWeatherRecordIds: [] } }; }
function dataConfidence(forecast: WeatherForecastSnapshot | null, relationship: RelationshipCardReadModel | null): ConfidenceLevel { if (!forecast || forecast.availability.status === 'UNAVAILABLE') return 'LOW'; const forecastConfidence: ConfidenceLevel = forecast.availability.status === 'AVAILABLE' ? 'HIGH' : 'MEDIUM'; if (!relationship) return forecastConfidence; return lowerConfidence(forecastConfidence, relationship.dataConfidence); }
function confidence(status: TomorrowFatiguePredictionReadModel['status'], forecast: WeatherForecastSnapshot | null, relationship: RelationshipCardReadModel | null): ConfidenceLevel { if (status === 'RAIN_NOT_EXPECTED' && forecast) return forecast.availability.status === 'AVAILABLE' ? 'MEDIUM' : 'LOW'; if (status !== 'OUTLOOK_AVAILABLE' || !forecast || !relationship) return 'LOW'; return forecast.availability.status === 'AVAILABLE' && relationship.analysisConfidence === 'HIGH' ? 'HIGH' : 'MEDIUM'; }
function lowerConfidence(left: ConfidenceLevel, right: ConfidenceLevel): ConfidenceLevel { const order: readonly ConfidenceLevel[] = ['LOW', 'MEDIUM', 'HIGH']; return order[Math.min(order.indexOf(left), order.indexOf(right))]; }
function selectLatestForecast(records: readonly WeatherForecastSnapshot[], targetDate: string, location: BaseLocation): WeatherForecastSnapshot | null { return records.filter((record) => record.source.sourceType === 'FORECAST' && record.targetPeriod.granularity === 'DAILY' && record.targetPeriod.localDate === targetDate && record.targetPeriod.timezone === location.timezone && sameLocation(record, location)).slice().sort((a, b) => compareRecency(b, a))[0] ?? null; }
function sameLocation(record: WeatherForecastSnapshot, location: BaseLocation) { return record.location !== null && record.location.timezone === location.timezone && record.location.latitude === location.coordinates.latitude && record.location.longitude === location.coordinates.longitude; }
