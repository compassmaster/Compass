...
export class HistoricalWeatherAcquisitionService {
  ...
  listLatest(limit=7): readonly ObservedWeatherRecord[] { const location = this.locations.get(); if (!location) return [];
    const locationSnapshot = toWeatherLocationSnapshot(location);
    const latest = new Map<string,ObservedWeatherRecord>(); for (const item of this.records.findAll().filter((record) => isRecordForLocation(record, locationSnapshot))) {
...
function isRecordForLocation(record: ObservedWeatherRecord, location: NonNullable<ObservedWeatherRecord['location']>): boolean {
  return record.source.sourceType === 'HISTORICAL'
    && record.observedPeriod.granularity === 'DAILY'
    && record.observedPeriod.timezone === location.timezone
    && sameLocationSnapshot(record.location, location);
}
