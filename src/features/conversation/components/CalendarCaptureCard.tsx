...
export function CalendarCaptureCard({ capture, onAnswer, onConfirm, onBeginEdit, onApplyEdit, onReject, onCancel, onCommit, onNavigate, onDismissReceipt }: Props) {
  ...
  if (candidate.status === 'COMMITTED' && candidate.receipt) return <section ref={reviewRef} tabIndex={-1} className="capture-review" aria-label="カレンダー保存結果"><h3>予定を保存しました</h3><p>保存先: Calendar</p><button type="button" onClick={() => onNavigate(candidate.receipt!)}>Calendarでこの予定を見る</button><button type="button" onClick={onDismissReceipt}>閉じる</button></section>;
  if (candidate.status === 'EDITING') return <form className="capture-review" onSubmit={e=>{e.preventDefault();setError(onApplyEdit(draft)??'');}}><h3>Candidateを修正</h3><label>予定名<input ref={titleRef} required value={draft.title} onChange={e=>setDraft({...draft,title:e.target.value})}/></label><label>メモ<textarea value={draft.note} onChange={e=>setDraft({...draft,note:e.target.value})}/></label><fieldset><legend>予定の種類</legend><label><input type="radio" checked={draft.timeKind==='ALL_DAY'} onChange={()=>setDraft({...draft,timeKind:'ALL_DAY'})}/>終日</label><label><input type="radio" checked={draft.timeKind==='TIMED'} onChange={()=>setDraft({...draft,timeKind:'TIMED'})}/>時刻指定</label></fieldset>{draft.timeKind==='ALL_DAY'?<><label>開始日<input ref={startDateRef} type="date" value={draft.startDate} onChange={e=>setDraft({...draft,startDate:e.target.value})}/></label><label>終了日<input ref={endDateRef} type="date" value={draft.endDate} onChange={e=>setDraft({...draft,endDate:e.target.value})}/></label></>:<><label>開始日時<input ref={startsAtRef} type="datetime-local" value={draft.startsAt} onChange={e=>setDraft({...draft,startsAt:e.target.value})}/></label><label>終了日時<input ref={endsAtRef} type="datetime-local" value={draft.endsAt} onChange={e=>setDraft({...draft,endsAt:e.target.value})}/></label><label>タイムゾーン<select ref={timeZoneRef} value={draft.timeZone} onChange={e=>setDraft({...draft,timeZone:e.target.value})}>{availableTimeZones(draft.timeZone).map(z=><option key={z}>{z}</option>)}</select></label></>}{error&&<p role="alert">{error}</p>}<button type="submit">修正内容を適用</button></form>;
  return <section ref={reviewRef} tabIndex={-1} className="capture-review" aria-label="カレンダー保存候補"><h3>Calendar専用Candidate</h3><p>保存先: Calendar</p><p>目的: 予定として保存</p><p>保存方法: 会話から保存（CONVERSATION_CAPTURE）</p><p>日時: {candidate.draft.timeKind==='ALL_DAY'?`${candidate.draft.startDate}〜${candidate.draft.endDate}（終日）`:`${candidate.draft.startsAt}〜${candidate.draft.endsAt} (${candidate.draft.timeZone})`}</p><p>入力元: 本人による構造化入力</p><p>元の発言: {candidate.sourceExcerpt}</p><p>状態: {candidate.status==='FAILED'?'保存失敗・未保存':candidate.status==='COMMITTING'?'保存中':'未保存'}</p><p>予定名: {candidate.draft.title}</p>{candidate.draft.note&&<p>メモ: {candidate.draft.note}</p>}<p>保持: Calendarから削除するまで保持されます。</p>{candidate.failure&&<p role="alert">保存できませんでした。候補を保持しています。再試行できます。</p>}<div className="capture-review-actions">{candidate.status==='PROPOSED'&&<button type="button" onClick={onConfirm}>この内容を確認する</button>}{candidate.status!=='FAILED'&&<button type="button" disabled={candidate.status==='COMMITTING'} onClick={onBeginEdit}>候補を修正する</button>}{(candidate.status==='READY'||candidate.status==='FAILED')&&<button ref={retryRef} type="button" onClick={onCommit}>{candidate.status==='FAILED'?'保存を再試行':'この内容で保存'}</button>}<button type="button" disabled={candidate.status==='COMMITTING'} onClick={onReject}>保存しない</button></div></section>;
}
