...
import { presentDailyLogCaptureFlowError } from './dailyLogCaptureFlowErrorPresentation.ts';
...
export function DailyLogCaptureFlowCard({ flow, onAnswer, onBack, onCancel }: Props) {
  ...
  const [error, setError] = useState<ReturnType<typeof presentDailyLogCaptureFlowError>>();
  ...
  const submit = (answer: DailyLogCaptureAnswer) => {
    ...
    if (!reason) return;
    const presented = presentDailyLogCaptureFlowError(reason);
    setError(presented);
    requestAnimationFrame(() => {
      if (presented.target === 'DATE' || presented.target === 'SCALE') firstControlRef.current?.focus();
    });
  };
  const fieldError = error?.target === 'DATE' || error?.target === 'SCALE' ? error : undefined;
  ...
  return <section className="daily-log-flow" aria-labelledby="daily-log-flow-title">
    ...
    {flow.step === 'DATE' && <div className="daily-log-flow-field">
      ...
      <input ref={firstControlRef as React.RefObject<HTMLInputElement>} id="daily-log-flow-date" type="date" value={text} aria-invalid={Boolean(fieldError)} aria-describedby={describedBy} onChange={(event) => { setText(event.target.value); setError(undefined); }} />
    ...
    {(flow.step === 'MOOD' || flow.step === 'FATIGUE') && <fieldset className="daily-log-flow-field" aria-describedby={describedBy}>
      ...
      <button type="button" onClick={() => scale === null ? submit(flow.step === 'MOOD' ? { step: 'MOOD', value: 0 } : { step: 'FATIGUE', value: 0 }) : submit(flow.step === 'MOOD' ? { step: 'MOOD', value: scale } : { step: 'FATIGUE', value: scale })}>次へ</button>
    </fieldset>}
    {flow.step === 'NOTE' && <div className="daily-log-flow-field">
      <label htmlFor="daily-log-flow-note">noteはありますか？</label>
      <textarea ref={firstControlRef as React.RefObject<HTMLTextAreaElement>} id="daily-log-flow-note" value={text} onChange={(event) => { setText(event.target.value); setError(undefined); }} />
    ...
    {flow.step === 'EVENTS' && <div className="daily-log-flow-field">
      ...
      <textarea ref={firstControlRef as React.RefObject<HTMLTextAreaElement>} id="daily-log-flow-events" value={text} onChange={(event) => { setText(event.target.value); setError(undefined); }} />
    ...
    {error && <p id={errorId} className="daily-log-flow-error" role="alert" aria-live="assertive">{error.message}</p>}
