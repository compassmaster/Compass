...
export function HomeSummaryPanel({ onNavigateToLog, onNavigateToSleep, onNavigateToWeather, onNavigateToPrediction }: Props) {
  ...
  return <section className="home-summary" aria-labelledby="home-summary-title">
    ...
    <div className="home-summary-grid">
      <article><h3>📝 今日の記録</h3>{latestLog ? <><strong>気分 {latestLog.mood} / 5</strong><strong>疲労 {latestLog.fatigue} / 5（高いほど疲れています）</strong><p>{today.dailyLogs.length > 1 ? `${today.dailyLogs.length}件のうち最新の記録` : '記録済み'}</p></> : <><strong>まだ記録がありません</strong><button type="button" onClick={onNavigateToLog}>今日を記録する</button></>}</article>
      <article><h3>🌙 睡眠</h3>{today.sleepRecord ? <><strong>{formatDurationMinutes(today.sleepRecord.durationMinutes)}</strong><p>起床日が今日の睡眠記録</p></> : <><strong>記録がありません</strong><p>睡眠時間は推測せず、未記録として表示します。</p></>}<button type="button" onClick={onNavigateToSleep}>睡眠記録を確認する</button></article>
      ...
      <article><h3>☂️ 明日の疲労見通し</h3><strong>{tomorrowOutlook.headline}</strong><p>{tomorrowOutlook.explanation}</p><small>{tomorrowOutlook.caution}</small><button type="button" onClick={onNavigateToPrediction}>明日の見通しを確認する</button></article>
    </div>
  </section>;
}
